...
def main_ploidy(cns_df, samples_df, cn_columns=None, segs=None, assembly=hg19, print_info=False):    
    ...
    res_df = normalize_feature(res_df, "ane_hom", assembly)
    res_df = calc_loh_bases(cns_df, res_df, cn_columns, assembly)
    res_df = normalize_feature(res_df, "loh_hom", assembly)
    if len(cn_columns) == 2:
        res_df = normalize_feature(res_df, "ane_het", assembly)
        res_df = normalize_feature(res_df, "loh_het", assembly)
        res_df = calc_imb_bases(cns_df, res_df, cn_columns, col_index=0, assembly=assembly)
        res_df = normalize_feature(res_df, f"imb_{cn_columns[0]}", assembly)
        if is_hap_spec(cn_columns):
            res_df = calc_imb_bases(cns_df, res_df, cn_columns, col_index=1, assembly=assembly)
            res_df = normalize_feature(res_df, f"imb_{cn_columns[1]}", assembly)
    return res_df
