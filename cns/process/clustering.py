...
def cluster_segments(input_segs, clust_dist, keep_ends = True, print_info=False):
    ...
    log_info(f"Merging {seg_count} segments with merge distance {clust_dist} ... ", suppress=not print_info)
    res = {}
    ...
    log_info(f"Removed {seg_count - new_count} segments by distance merge.", suppress=not print_info)
    log_info(f"Resulting segment count: {new_count}", suppress=not print_info)
    return res