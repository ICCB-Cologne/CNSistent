...
def get_ane_for_col(col, row, samples_df, assembly=hg19):
    return get_expected_ploidy(col, row["chrom"], samples_df.loc[row["sample_id"]]["sex"] == "xy", assembly) != row[col] and not np.isnan(row[col])
...
def _calc_bases_per_column(res, cns_df, samples_df, cn_columns, het, feature, assembly=hg19):
    ...
    mask = function(cns_df, samples_df, cn_columns, het, assembly)
...
def calc_loh_bases(cns_df, samples_df, cn_columns, assembly=hg19):
    res = samples_df.copy()
    if len(cn_columns) == 2:
        res = _calc_bases_per_column(res, cns_df, samples_df, cn_columns, True, "loh", assembly)
    res = _calc_bases_per_column(res, cns_df, samples_df, cn_columns, False, "loh", assembly)    
    return res
